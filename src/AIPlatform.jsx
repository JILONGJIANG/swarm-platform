...
function PhoneHubEmbed({hubUrl}) {
  return (
    <iframe
      src={hubUrl}
      style={{width:'100%', height:'calc(100vh - 120px)', border:'none', borderRadius:'8px', background:'#fff'}}
      title="Phone Hub"
    />
  );
}
...
function PhoneHubPanel() {
  const [hubUrl, setHubUrl] = React.useState('http://192.168.1.110:5050');
  React.useEffect(() => {
    fetch('/api/phone/status').then(r=>r.json())
      .then(d => { if(d.hubs?.[0]?.url) setHubUrl(d.hubs[0].url); })
      .catch(() => {});
  }, []);
  return <PhoneHubEmbed hubUrl={hubUrl} />;
}
